...
class LeadtrekkerWebformHandler extends WebformHandlerBase {
  ...
  protected function remotePost($operation, WebformSubmissionInterface $webform_submission) {
    ...
    $form_guid = $this->connection->select('leadtrekker', 'h')
      ->fields('h', ['leadtrekker_guid'])
      ->condition('id', $id)
      ->range(0, 1)
      ->execute()->fetchField();

    $portal_id = $this->configFactory->get('leadtrekker_portalid');

    $api = 'https://forms.leadtrekker.com/uploads/form/v2/' . $portal_id . '/' . $form_guid;
    ...
    $cookie = \Drupal::request()->cookies->get('leadtrekkerutk');
    ...
    try {
      ...
      $leadtrekker_url = 'https://app.leadtrekker.com';
      $to = $this->configFactory->get('leadtrekker_debug_email');
      $default_language = \Drupal::languageManager()->getDefaultLanguage()->getId();
      $from = $this->configFactory->get('site_mail');
      $data = (string) $response->getBody();

      if ($response->getStatusCode() == '204') {
        $this->loggerFactory->get('leadtrekker')->notice('Webform "%form" results succesfully submitted to Leadtrekker. Response: @msg', [
          '@msg' => strip_tags($data),
          '%form' => $form_title,
        ]
        );
      ...
      elseif (!empty($response['Error'])) {
        ...
        if ($this->configFactory->get('leadtrekker_debug_on')) {
          $this->mailManager->mail('leadtrekker', 'http_error', $to, $default_language, [
            'errormsg' => $response['Error'],
            'leadtrekker_url' => $leadtrekker_url,
        ...
        if ($this->configFactory->get('leadtrekker_debug_on')) {
          $this->mailManager->mail('leadtrekker', 'hub_error', $to, $default_language, [
            'errormsg' => $data,
            'leadtrekker_url' => $leadtrekker_url,
            'node_title' => $form_title,
          ], $from);
